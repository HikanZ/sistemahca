...
if ($uidSetor == "Todos os setores"){
  $sql = "SELECT * FROM audit WHERE versionRop=? AND yearAudit=? AND monthAudit=?";
}else{
  $sql = "SELECT * FROM audit WHERE versionRop=? AND idSetor=? AND yearAudit=? AND monthAudit=?";
}
$stmt = mysqli_stmt_init($conn); //Aqui faz a conexão com o banco
if (!mysqli_stmt_prepare($stmt, $sql)) { //Se houver algum erro de sql
  ...
  if ($uidSetor == "Todos os setores"){
    mysqli_stmt_bind_param($stmt, "iis", $version, $anoSelecionado, $_POST['mes']);
  }else{
    mysqli_stmt_bind_param($stmt, "iiis", $version, $idSetor, $anoSelecionado, $_POST['mes']);
  }
  mysqli_stmt_execute($stmt);
