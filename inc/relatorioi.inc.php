...
$anoi = $_POST['anoSelecionadoIni'];
$mesi = $_POST['mesIni'];
$anof = $_POST['anoSelecionadoFim'];
$mesf = $_POST['mesFim'];

$datai = "$anoi.'-'.$mesi.'-00 00:00:00'";
$dataf = "$anof.'-'.$mesf.'-31 23:59:59'";
...
if (isset($_POST['setor'])){
  if ($_POST['setor']=='ALL'){
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) { //Se houver algum erro de sql
      header("Location: relatorio-intervalo.php?error=connectionerror3"); //Retornará à pag anterior
      exit();
    }
...
$stmt = mysqli_stmt_init($conn); //Aqui faz a conexão com o banco
if (!mysqli_stmt_prepare($stmt, $sql)) { //Se houver algum erro de sql
  header("Location: relatorio-intervalo.php?error=connectionerror4"); //Retornará à pag anterior
  exit();
}
else{ //Se a conexão for bem sucedida, fará a consulta
  mysqli_stmt_execute($stmt);
...
if (!mysqli_stmt_prepare($stmtG, $sql)) { //Se houver algum erro de sql
  header("Location: relatorio-intervalo.php?error=sqlerror5"); //Retornará à pag anterior
  exit();
}
...
$stmt = mysqli_stmt_init($conn); //Aqui faz a conexão com o banco
if (!mysqli_stmt_prepare($stmt, $sql)) { //Se houver algum erro de sql
  header("Location: relatorio-intervalo.php?error=sqlerror6"); //Retornará à pag anterior
  exit();
}
else{ //Se a conexão for bem sucedida, fará a verificação
...
if ($uidSetor == "Todos os setores"){
  $sql = "SELECT * FROM answer INNER JOIN audit ON answer.idAudit = audit.idAudit WHERE audit.startAudit BETWEEN ? AND ? AND versionAudit=?";
}else{
  $sql = "SELECT * FROM answer INNER JOIN audit ON answer.idAudit = audit.idAudit WHERE audit.startAudit BETWEEN ? AND ? AND versionAudit=? AND idSetor=?";
}

$stmt = mysqli_stmt_init($conn); //Aqui faz a conexão com o banco
if (!mysqli_stmt_prepare($stmt, $sql)) { //Se houver algum erro de sql
  header("Location: relatorio-intervalo.php?error=sqlerror7"); //Retornará à pag anterior
  exit();
}else{ //Se a conexão for bem sucedida, fará a verificação
  if ($uidSetor == "Todos os setores"){
...
if ($uidSetor == "Todos os setores"){
  $sql = "SELECT * FROM audit WHERE startAudit BETWEEN ? AND ? AND versionRop=?";
}else{
  $sql = "SELECT * FROM audit WHERE startAudit BETWEEN ? AND ? AND versionRop=? AND idSetor=?";
}
$stmt = mysqli_stmt_init($conn); //Aqui faz a conexão com o banco
if (!mysqli_stmt_prepare($stmt, $sql)) { //Se houver algum erro de sql
  header("Location: relatorio-intervalo.php?error=sqlerror8"); //Retornará à pag anterior
  exit();
}else{ //Se a conexão for bem sucedida, fará a verificação
  if ($uidSetor == "Todos os setores"){
    mysqli_stmt_bind_param($stmt, "ssi", $datai, $dataf, $version);
...
if ($uidSetor == "Todos os setores"){
  ...
  $sql = "SELECT answer.monthAudit FROM answer INNER JOIN audit ON answer.idAudit = audit.idAudit WHERE audit.startAudit BETWEEN ? AND ? AND answer.yearAudit=? AND idSetor=? GROUP BY answer.monthAudit ORDER BY answer.monthAudit ASC";
}

$stmt = mysqli_stmt_init($conn); //Aqui faz a conexão com o banco
if (!mysqli_stmt_prepare($stmt, $sql)) { //Se houver algum erro de sql
  header("Location: relatorio-intervalo.php?error=sqlerror9"); //Retornará à pag anterior
  exit();
}else{ //Se a conexão for bem sucedida, fará a verificação
  if ($uidSetor == "Todos os setores"){
    mysqli_stmt_bind_param($stmt, "ssi", $datai, $dataf, $version);
