$(document).ready(function() {
    ...
    $('#search-val-name, #datauditid, #cpf2, #cargo, #default-select2, #iduserid').keyup(function() {
        checkAll();
    });
    ...
    function checkAll() {
        ...
        $('#resultTable tr:not(:first-child)').each(function(i, item) {
            if (
                ...
                $(this).find('td:eq(1)').text().toLowerCase().indexOf($('#cpf2').val().toLowerCase()) >= 0 &&
              	// 2.) check for text match...
                $(this).find('td:eq(2)').text().toLowerCase().indexOf($('#datauditid').val().toLowerCase()) >= 0 &&
                $(this).find('td:eq(3)').text().toLowerCase().indexOf($('#cargo').val().toLowerCase()) >= 0 &&
                $(this).find('td:eq(4)').text().toLowerCase().indexOf($('#iduserid').val().toLowerCase()) >= 0 &&
